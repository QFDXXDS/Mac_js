...
@synthesize host, token, displayName, resourceId, chatMessage;
...
//    vc                  = nil;
    ...
    mainView.layer.backgroundColor = [NSColor redColor].CGColor ;
...
//    [CHITVideoService initialize:YES view:self];
...
//    [host setStringValue:[appSettings host]];
//    [token setStringValue:[appSettings token]];
//    [displayName setStringValue:[appSettings displayName]];
//    [resourceId setStringValue:[appSettings resourceId]];
//
//    [host setStringValue:@"vcc.changhongit.com"];
//    [token setStringValue:@"Aj6oFNasUV"];
//    [displayName setStringValue:[appSettings displayName]];
//
//
//
...
//     the previewView or the mainView, which is the full screen other than bottom toolbar.
    ...
    if ( ( vidyoConnectorState == VidyoConnectorStateConnected ) || [appSettings hideConfig] ) {
        ...
        [[CHITVideoService connector] showViewAt:&mainView X:0 Y:0 Width:(mainView.frame.size.width - chatWidth) Height:mainView.frame.size.height];
        
        NSLog(@"执行refreshUI --VidyoConnectorStateConnected");

    } else {
        [[CHITVideoService connector] showViewAt:&mainView X:previewView.frame.origin.x Y:previewView.frame.origin.y Width:previewView.frame.size.width Height:previewView.frame.size.height];
        NSLog(@"执行refreshUI -- else ");
...
- (void)connect:( id )sender {
    ...
    } else if ( [[resourceId stringValue] containsString:@" "] || [[resourceId stringValue] containsString:@"@"] ) {
        // Abort the Connect call if resourceId is invalid. It cannot contain empty spaces or "@".
        [controlsStatusText setStringValue:@"Connect aborted due to invalid Resource ID"];
        [controlsStatusText setTextColor:[NSColor redColor]];
    } else {
        // Attempt to connect
...
//            status = [vc connect:[[host stringValue] UTF8String]
//                          Token:[[token stringValue] UTF8String]
//                          DisplayName:[[displayName stringValue] UTF8String]
//                          ResourceId:[[resourceId stringValue] UTF8String]
//                          ConnectorIConnect:self];
        } else {
            // Connect to a VidyoCloud system, not Vidyo.io.
//            status = [vc connectToRoomAsGuest:[[appSettings portal] UTF8String]
//                          DisplayName:[[displayName stringValue] UTF8String]
//                          RoomKey:[[appSettings roomKey] UTF8String]
//                          RoomPin:[[appSettings roomPin] UTF8String]
//                          ConnectorIConnect:self];
            
//           status = [vc connectToRoomAsGuest:[[host stringValue] UTF8String]
//                                      DisplayName:[[displayName stringValue] UTF8String]
//                                      RoomKey:[[token stringValue] UTF8String]
//                                      RoomPin:nil
//                                      ConnectorIConnect:self];
...
- (void)onLocalCameraSelected:(VCLocalCamera *)camera {
    ...
    NSLog(@"当前CameraSelected is %@", camera);
